...
public class Solution {
    ...
    public static Optional<Car> getMostExpensiveCar(Stream<Car> cars) {
        Optional<Car> result = cars.max( (v1, v2) -> v1.getPrice() - v2.getPrice() );
        return result;
    }

    public static Optional<Car> getMoreExpensiveCar(Stream<Car> cars, Car mostExpensiveCar) {
        Stream<Car> result = cars.filter(x -> (x.getPrice() > mostExpensiveCar.getPrice()));
        Optional<Car> result2 = result.findFirst();
        return result2;
    }
}
